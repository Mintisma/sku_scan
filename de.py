...
def get_asins_de():
    ...
    # get data
    inser_sql = """
        select * from product_to_scrape where country = 'de';
    """
...
def get_info(asin):
    ...
    def get_rank_small(selector):
        try:
            ...
            rank_small = int(rank_span[1].xpath('text()').extract_first().split()[0].replace('#', '').replace(',', ''))
        except Exception as e:
            try:
                rank_span = selector.xpath('//*[contains(text(),"Best Sellers Rank")]/parent::li')
                rank_small = int(rank_span.xpath('ul/li/span/text()').extract_first().replace('#', ''))
            except Exception as e:
                print(e)
                print('rank_small')
                rank_small = 999999
    ...
    def get_rank_large(selector):
        try:
            rank_span = selector.xpath('//th[contains(text(),"Best Sellers Rank")]/parent::tr/td/span/span')
            rank_large = int(rank_span[0].xpath('text()').extract_first().split()[0].replace('#', '').replace(',', ''))
        except Exception as e:
            try:
                rank_span = selector.xpath('//*[contains(text(),"Best Sellers Rank")]/parent::li')
                rank_large = int(rank_span.xpath('text()')[1].extract().strip().split()[0].replace('#', '').replace(',', ''))
            except Exception as e:
                print(e)
                print('rank_large')
                rank_large = 1
