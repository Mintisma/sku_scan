...
def get_asins_jp():
    ...
    # get data
    inser_sql = """
        select * from product_to_scrape where country = 'jp';
    """
...
def get_info(asin):
    ...
    def get_rank_large(selector):
        try:
            rank_large = int(
                selector.xpath('//*[contains(text(), "Amazon 売れ筋ランキング")]/../text()[2]').extract_first().strip().split()[
                    2].replace(',', '').replace('位', ''))
        except:
            try:
                rank_large = int(selector.xpath(
                    '//*[contains(text(), "Amazon 売れ筋ランキング")]/../td[2]/text()').extract_first().strip().split()[
                                     2].replace(',', '').replace('位', ''))
    ...
    def get_rank_small(selector):
        try:
            rank_small = int(selector.xpath(
                '//*[contains(text(), "Amazon 売れ筋ランキング")]/../ul/li[1]/span[1]/text()').extract_first().replace(',',
                                                                                                               '').replace(
                '位', ''))
        except:
            try:
                rank_small = int(selector.xpath(
                    '//*[contains(text(), "Amazon 売れ筋ランキング")]/../td/ul/li[1]/span[1]/text()').extract_first().replace(
                    ',', '').replace('位', ''))
            except Exception as e:
                print(e)
                print('rank_small')
    ...
    def get_review(selector):
        try:
            review = int(
                selector.xpath('//span[@id="acrCustomerReviewText"]/text()').extract_first('').replace('件のカスタマーレビュー',
                                                                                                       '').replace(',',
                                                                                                                   ''))
        except Exception as e:
            review = 0
    ...
    def get_price(selector):
        try:
            price = float(
                selector.xpath('//span[@id="priceblock_ourprice"]/text()').extract_first('').replace('￥ ', '').replace(
                    ',', ''))
        except Exception as e:
            try:
                price = float(
    ...
    def insert_data(data):
        ...
        cursor = conn.cursor()
        insert_sql = """
            insert into jp (`asin`, `rank_large`, `rank_small`, `review`, `price`, `star`, `category`, `sku`, `scrape_time`, `store`, `country`) values
