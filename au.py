...
def get_asins_au():
    ...
    # get data
    inser_sql = """
        select * from product_to_scrape where country = 'au';
    """
...
def get_info(asin):
    ...
    def get_rank_large(selector):
        try:
            rank_large = int(
                selector.xpath('//td[contains(text(),"Best Sellers Rank")]/following-sibling::td/text()').extract_first(
                    '').strip().split()[0].replace('#', '').replace(',', ''))
        except Exception as e:
            try:
                rank_large = int(
                    selector.xpath('//*[contains(text(), "Bestsellers Rank")]/parent::li/text()').extract()[
                        1].strip().split()[0])
            except Exception as e:
                print(e)
                print('rank_large')
    ...
    def get_rank_small(selector):
        try:
            rank_small = int(selector.xpath(
                '//td[contains(text(),"Best Sellers Rank")]/following-sibling::td/ul/li/span/text()').extract_first(
                '').replace('#', '').replace(',', ''))
        except Exception as e:
            try:
                rank_small = int(selector.xpath(
                    '//*[contains(text(), "Bestsellers Rank")]/parent::li/ul/li/span/text()').extract_first('').replace(
                    '#', '').replace(',', ''))
            except Exception as e:
                print(e)
                print('rank_small')
    ...
    def insert_data(data):
        ...
        cursor = conn.cursor()
        insert_sql = """
            insert into au (`asin`, `rank_large`, `rank_small`, `review`, `price`, `star`, `category`, `sku`, `scrape_time`, `store`, `country`) values
