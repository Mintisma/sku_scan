...
def get_asins_fr():
    ...
    # get data
    inser_sql = """
        select * from product_to_scrape where country = 'fr';
    """
...
def get_info(asin):
    ...
    def get_rank_large(selector):
        try:
            rank_span = selector.xpath('//*[contains(text(),"Classement des meilleures ventes")]/parent::li')
            rank_large = rank_span.xpath('text()').extract()[1].strip().split()[0].replace('.', '')
        except Exception as e:
            try:
                rank_span = selector.xpath('//*[contains(text(),"Classement des meilleures ventes")]/parent::tr/td')
                rank_large = rank_span.xpath('text()').extract()[1].strip().split()[0].replace('.', '')
            except Exception as e:
                print(e)
                rank_large = 1
    ...
    def get_rank_small(selector):
        try:
            rank_span = selector.xpath('//*[contains(text(),"Classement des meilleures ventes")]/parent::li')
            rank_small = rank_span.xpath('ul/li/span/text()').extract_first().replace('n°', '')
        except Exception as e:
            try:
                rank_span = selector.xpath('//*[contains(text(),"Classement des meilleures ventes")]/parent::tr/td')
                rank_small = rank_span.xpath('ul/li/span/text()').extract_first().replace('n°', '')
    ...
    def get_price(selector):
        try:
            price = float(selector.xpath('//span[@id="priceblock_saleprice"]/text()').extract_first('').replace('EUR ', '').replace(',', '.'))
        except Exception as e:
            try:
                price = float(selector.xpath('//span[@id="priceblock_ourprice"]/text()').extract_first('').replace('EUR ', '').replace(',', '.'))
            except Exception as e:
                print(e)
                price = 999
        return price
